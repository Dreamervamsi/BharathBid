...
export default function Templates() {
  ...
  const handleDuplicate = (title) => {
    showToast(`Template "${title}" duplicated to drafts!`, 'info');
  };

  const handleDownload = (title) => {
    showToast(`Downloading PDF for "${title}"...`, 'success');
  };

  const handleCreateNew = () => {
    showToast('Opening template editor for new notice...', 'info');
  };
    ...
    <div className="space-y-4 animate-fade-up">
      <div className="bg-white p-4 rounded-lg border border-slate-200 shadow-2xs flex items-center justify-between">
        ...
        <button
          ...
          className="py-2 px-3 bg-blue-600 hover:bg-blue-700 text-white rounded text-xs font-bold flex items-center space-x-1.5 shadow-2xs transition-colors"
        >
          ...
          <div key={t.id} className="bg-white p-4 rounded-lg border border-slate-200 shadow-2xs space-y-3">
            ...
            <div className="pt-2 border-t border-slate-100 flex items-center justify-between text-xs">
              ...
              <div className="flex items-center space-x-2">
                <button
                  onClick={() => handleDuplicate(t.title)}
                  className="py-1 px-2.5 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded text-[11px] font-bold flex items-center space-x-1 transition-colors"
                >
                ...
                <button
                  ...
                  className="py-1 px-2.5 bg-slate-900 hover:bg-slate-800 text-white rounded text-[11px] font-bold flex items-center space-x-1 transition-colors"
                >
