    ...
    <div className="min-h-screen bg-slate-900 flex flex-col justify-center py-12 sm:px-6 lg:px-8 font-sans select-none">
      <div className="sm:mx-auto sm:w-full sm:max-w-md text-center">
        ...
        <h2 className="text-xl font-black text-white tracking-tight uppercase">
          GeM Forensic Verification
        </h2>
        <p className="mt-1 text-xs text-slate-400 font-medium">
          Procurement Compliance Audit Portal • Government of India
        </p>
      </div>
      ...
      <div className="mt-6 sm:mx-auto sm:w-full sm:max-w-md">
          ...
          <form className="space-y-4" onSubmit={handleSubmit}>
            ...
            <div>
              <button
                type="submit"
                disabled={loading}
                className="w-full py-2.5 px-4 border border-transparent rounded-md shadow-xs text-xs font-bold text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 flex items-center justify-center space-x-2 transition-all"
              >
                {loading ? <Loader2 className="w-4 h-4 animate-spin text-white" /> : <ShieldCheck className="w-4 h-4 text-white" />}
                <span>Sign In to Audit Portal</span>
              </button>
            </div>
          </form>

          <div className="mt-6 pt-4 border-t border-slate-100 flex items-center justify-between text-xs">
            <span className="text-slate-500 font-medium">New procurement officer?</span>
            <Link to="/register" className="font-bold text-blue-600 hover:text-blue-700 flex items-center gap-1">
        ...
        <div className="mt-4 text-center text-[10px] text-slate-500">
          Secure Multi-Factor Forensic Verification Engine • Government Procurement
        </div>
      </div>
    </div>
  );
